...
SYSTEM = (
    ...
    "• SUPPORT theo vật liệu (hub tự sinh 'Cách làm support'): (a) CÙNG loại — PLA để "
    "Top Z 0.15 (mặt đẹp) / 0.25 (dễ gỡ); PETG PHẢI để Top Z 0.3–0.4 (1.5–2× layer) vì "
    "PETG hàn chính nó mạnh hơn PLA, để 0.15 là DÍNH CHẾT — kèm Base pattern Gyroid "
    "(zig-zag PETG khó bẻ) + interface 30–40mm/s. (b) KHÁC loại (PLA↔PETG KHÔNG dính hoá "
    "học): interface = nhựa kia, Top Z=0, 1 lớp concentric → bóc SẠCH, mặt dưới nhẵn như "
    "mặt trên; nhớ FLUSH nhiều khi đổi (PLA→PETG ~650, PETG→PLA ~250).\n"
